	...
	UnityLight light, UnityIndirect gi)
{
	...
	// we tweak a2 a bit to "confuse" compiler and force clamp
#if defined (SHADER_API_MOBILE)
	half numerLinear = a2 - 1e-5h, numerGamma = a - 1e-5h;
#else
	half numerLinear = a2, numerGamma = a;
#endif
	...
	half specularTerm;
	if(IsGammaSpace())
		specularTerm = numerGamma / (max(0.32h, lh) * (1.5h + roughness) * d);
	else
		specularTerm = numerLinear / (max(0.1h, lh*lh) * (roughness + 0.5h) * (d * d) * 4);
